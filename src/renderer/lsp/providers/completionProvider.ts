        ...
        provideCompletionItems: async (model, position) => {
            ...
            try {
                ...
                const suggestions = (items as Array<Record<string, unknown>>).map(
                    (item) => mapItem(item, monaco, range)
                )
                return { suggestions }
            } catch {
                return { suggestions: [] }
...
export const createCompletionProvider = (monaco: MonacoInstance): monacoLanguages.CompletionItemProvider =>
    makeCompletionProvider(monaco, ['\\', '{', ',', ' '], mapLatexItem)

export const createBibtexCompletionProvider = (monaco: MonacoInstance): monacoLanguages.CompletionItemProvider =>
