...
test('PDF evidence reads actual page text, persists excerpts and detects replaced bytes', async ({ page }) => {
  await page.goto('/?evidence')
  await page.getByRole('button', { name: 'Link PDF evidence' }).click()
  await page.getByRole('combobox', { name: 'Reference PDF', exact: true }).selectOption('reference.pdf')
  await page.getByRole('button', { name: 'Read page', exact: true }).click()
  await expect(page.getByRole('textbox', { name: 'Page text · select an excerpt', exact: true })).toHaveValue('The efficient method works.')
  await page.getByRole('textbox', { name: 'Excerpt', exact: true }).fill('A fabricated finding')
  await expect(page.getByRole('button', { name: 'Save excerpt' })).toBeDisabled()
  await page.getByRole('textbox', { name: 'Excerpt', exact: true }).fill('efficient method')
  await page.getByRole('button', { name: 'Save excerpt' }).click()
  await expect(page.locator('.citation-evidence-record')).toContainText('efficient method')
  await expect(page.locator('.citation-evidence-record')).toContainText('matched the page text')
  await page.reload()
  await page.getByRole('button', { name: 'Link PDF evidence' }).click()
  await expect(page.locator('.citation-evidence-record')).toContainText('Saved excerpt · not rechecked')
  await page.getByRole('button', { name: 'Replace reference PDF' }).click()
  await page.getByRole('button', { name: 'Open page and recheck' }).click()
  await expect(page.locator('.citation-evidence-record')).toContainText('The PDF has changed since saving.')
  await expect(page.getByRole('textbox', { name: 'Page text · select an excerpt', exact: true })).toHaveValue('The revised method works.')
  await page.getByRole('button', { name: 'Remove excerpt' }).click()
  await expect(page.locator('.citation-evidence-record')).toHaveCount(0)
})
