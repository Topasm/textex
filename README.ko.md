...
- **PDF 작업 모드**: 상단 **PDF** 버튼을 누르면 원문과 보조 패널을 숨기고 PDF만 표시합니다. 문장 더블클릭으로 직접 수정하거나 AI로 다듬고, 수식·표·그림·인용·문서 구조는 **원문 편집** 패널에서 수정한 뒤 **저장하고 PDF 갱신**을 누르세요. 편집 상태와 실행 취소 기록은 유지됩니다. 파일 탐색과 연구 패널은 상단에서 다시 열고 **패널 숨기기**로 닫을 수 있습니다. 프로젝트의 TeX 원문을 수정해 재컴파일하는 기능이며, 대응하는 원문이 없는 임의의 PDF는 이 방식으로 편집할 수 없습니다.
- **PDF에서 문장 수정**: PDF 문장을 더블클릭하면 원문이 채워진 편집창이 열립니다. **직접 다듬기** 또는 **AI로 다듬기**를 선택하고 내용을 확인한 뒤 **적용하고 PDF 갱신**을 누르세요. 대응하는 TeX 문장만 변경하고, 해당 PDF의 원본 문서를 다시 컴파일합니다. 입력란의 LaTeX 서식 명령은 유지해 주세요. AI 기능은 설정에서 활성화해야 하며 제안은 자동 적용되지 않습니다. 같은 창에서 이번 수정을 되돌릴 수 있습니다. 문장 대응이 불확실하거나 원문이 바뀌면 추측한 범위를 덮어쓰지 않도록 편집을 제한합니다.
