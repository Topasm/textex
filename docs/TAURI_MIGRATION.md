...
project bibliography/Zotero, spellcheck, export, updater, external
links, and performance memory sampling.
Native AI HTTP/credential handling, isolated Claude Code/Codex CLI transformations,
and safe system-terminal launch are also implemented. TextEx does not embed a PTY or
external language server. The AI CLIs remain optional external tools. Do not
reintroduce a Node.js desktop backend as a fallback.
